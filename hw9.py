...
print(questify(['hi','thigh', 'sigh']))

def questifyAlt(str_list):
    '''Same as questify'''
    return map(lambda x: x + '?', str_list)

print(questifyAlt(['hi','thigh', 'sigh']))
...
def wordScoreLoop(S, scorelist):
    ...
    final = 0
    for i in S:
        final = letterScore(i, scrabbleScores) + final
    return final

print(wordScoreLoop('fly', scrabbleScores))
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
' Exercise 5
...
print(wordsWithScore(['python','is','awesome'], scrabbleScores))

def wordsWithScoreLambda(dct, scores):
    '''Same as wordsWithScore'''
    return map(lambda x: [x, wordScore(x, scrabbleScores)], dct)
...
def wordsWithScoreLoop(dct, scores):
    ...
    out = []
    for i in dct:
        out = out + [[i, wordScoreLoop(i, scrabbleScores)]]
    return out
